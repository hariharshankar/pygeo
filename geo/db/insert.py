...
class InsertFactSheet():
    ...
    def insert(self, table_name, form_data, module_type, description_id=0):
        ...
        if not module_type or module_type == "generic":
            return self.__insert_generic_module(table_name, form_data, description_id)
        elif module_type == "row_columns":
            return self.__insert_row_column_module(table_name, form_data, description_id)
        elif module_type == "performance":
            return self.__insert_performance_module(table_name, form_data, description_id)
    ...
    def __insert_history_module(self, form_data):
        ...
        if int(parent_plant_id) == 0:
            ...
            result = session.execute(parent_update_stmt,
    ...
    def __insert_performance_module(self, table_name, form_data, description_id):
        ...
        for year in range(start_decade, end_decade):
            ...
            sql_fields.extend(["`Description_ID`=:description_id", "`Year_yr`=:year_yr"])
            sql_values['description_id'] = description_id
    ...
    def __insert_row_column_module(self, table_name, form_data, description_id):
        ...
        for no in range(1, int(number_of_rows)+1):
            sql_values = {}
            sql_statement = []
            sql_fields = []
